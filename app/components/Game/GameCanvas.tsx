...
const GameCanvas: React.FC<GameCanvasProps> = ({ gameState, setGameState, jumpModifier }) => {
  ...
  useEffect(() => {
    const img = new Image();
    img.src = 'https://github.com/ProferredSolutions.png';
    img.onload = () => {
  ...
  const drawHumanoid = (ctx: CanvasRenderingContext2D, p: PlayerState, cameraX: number, cameraY: number) => {
    ...
    // If avatar image is loaded, draw it; otherwise fall back to simple humanoid
    if (playerAvatarRef.current) {
      // Draw the avatar image centered on the player position
      ctx.save();
      // Flip the image if facing left
      if (p.facing === -1) {
        ctx.translate(cx, y + p.height / 2);
        ctx.scale(-1, 1);
        ctx.drawImage(playerAvatarRef.current, -p.width / 2, -p.height / 2, p.width, p.height);
      } else {
        ctx.drawImage(playerAvatarRef.current, cx - p.width / 2, y, p.width, p.height);
      }
      ctx.restore();
    } else {
      // Fallback: Simple humanoid drawing
      // Body Color
